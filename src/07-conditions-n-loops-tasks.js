...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length - 1; i += 1) {
    ...
    if (!newStr.includes(str[i]) && !arr.includes(str[i])) {
      return str[i];
    }
    arr.push(str[i]);
  }
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = num - 1; i >= 0; i -= 1) {
    if (j % 2 === 0) sum += +str[i];
      ...
      const temp = (str[i] * 2).toString();
      sum += temp >= 10 ? +temp[0] + +temp[1] : +temp;
    }
    j += 1;
  }
  if (sum % 10 === 0) return sum % 10 === +str[num];
  return 10 - (sum % 10) === +str[num];
}
...
function getDigitalRoot(num) {
  ...
  return sum > 10 ? +sum.toString()[0] + +sum.toString()[1] : sum;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (Object.keys(brackets).includes(arr[i])) temp.push(arr[i]);
    else if (arr[i] === brackets[temp[temp.length - 1]]) temp.pop();
    else return false;
  }
